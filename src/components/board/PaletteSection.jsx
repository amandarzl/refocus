...
export default function PaletteSection({ isDark, entries, onRemove }) {
        ...
        {entries.map((entry) => (
          <div key={entry.folderId} className="flex items-center gap-3">
            <div className="flex flex-1 flex-wrap items-center gap-3">
              <span className={`w-20 flex-shrink-0 truncate text-sm font-semibold ${isDark ? "text-slate-300" : "text-slate-600"}`}>
                {entry.folderName}
              </span>
            ...
            <button
              ...
              className={`flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full text-xs transition-colors ${
                isDark ? "text-zinc-500 hover:bg-zinc-800 hover:text-zinc-300" : "text-slate-400 hover:bg-slate-100 hover:text-slate-600"
              }`}
            >
              ✕
            </button>
